...
import { User as UserIcon, MapPin, Camera, Save, ArrowLeft, Star, Award, ShieldCheck, Shield } from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ user, profile, onUpdateUser }) => {
          ...
          <div className="absolute -bottom-16 left-12">
            <div className="relative group">
              <div className="w-32 h-32 rounded-[2.5rem] bg-white p-1.5 shadow-2xl overflow-hidden border border-slate-100">
                <img src={profile?.avatar_url || `https://ui-avatars.com/api/?name=${profile?.full_name || 'U'}`} alt="User" className="w-full h-full object-cover rounded-[2rem]" />
              </div>
              <button className="absolute bottom-2 right-2 p-2.5 bg-orange-500 text-white rounded-xl shadow-lg hover:scale-110 transition-all">
                <Camera size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
            <div>
              ...
              <p className="text-slate-400 font-bold uppercase tracking-widest text-xs mt-1">
                Verified Professional Since 2025
              </p>
            </div>
            <button 
              onClick={() => isEditing ? handleSave() : setIsEditing(true)}
              disabled={loading}
              className="flex items-center gap-3 px-8 py-3.5 bg-[#1a2332] text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-50"
            >
            ...
            <div className="lg:col-span-2 space-y-10">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Work Email</label>
                    <input 
                      type="email" 
                      disabled
                      value={user?.email}
                      className="w-full px-0 py-4 rounded-2xl font-bold text-slate-400 bg-transparent border-2 border-transparent outline-none"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <textarea 
                  ...
                  placeholder="Professional experience, specialization, etc."
              ...
              <div className="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-100">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-black text-[#1a2332]">{profile?.rating || '5.0'} / 5.0</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-black text-[#1a2332] uppercase tracking-tighter">Deals</span>
                    </div>
                    <span className="text-lg font-black text-[#1a2332]">{profile?.total_transactions || '0'}</span>
                  </div>
                </div>
              </div>

              <div className="bg-[#1a2332] rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                <Shield className="absolute bottom-[-20px] right-[-20px] w-32 h-32 opacity-10 text-white" />
